...
<template>
  <BaseCard class="clip-card relative" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave">
    <div v-if="clip.published" class="absolute top-2 left-2 z-10">
      <BaseBadge class="bg-green-100 text-green-800 border-green-200 dark:bg-green-900/50 dark:text-green-300 dark:border-green-800">Published</BaseBadge>
    </div>
    ...
    <div class="p-4 space-y-3">
      <div class="flex items-center gap-2">
        <input class="w-full rounded-xl border border-border bg-white/70 dark:bg-white/10 px-3 h-10 outline-none flex-1" :value="clip.displayName ?? ''" :placeholder="clip.filename" @change="updateName" />
        <BasePopover side="bottom" :side-offset="8">
          <template #trigger>
            <button class="rounded-xl inline-flex items-center justify-center border border-border bg-white/70 dark:bg-white/10 px-3 py-2 outline-none size-10">
