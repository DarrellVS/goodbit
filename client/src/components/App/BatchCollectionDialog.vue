        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div v-if="showNewCollectionInput" class="space-y-2">
            <input
              ...
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent dark:border-slate-700"
          ...
          <button
            v-else
            class="w-full px-4 py-3 rounded-lg border-2 border-dashed border-gray-300 text-gray-600 hover:border-orange-500 hover:text-orange-600 transition-colors flex items-center justify-center gap-2 font-medium dark:border-slate-700 dark:text-slate-400"
          ...
          <div v-if="collectionsStore.items.length > 0" class="space-y-2">
            ...
            <div class="space-y-2">
              <button
                ...
                :key="collection.id"
                class="w-full px-4 py-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors flex items-center justify-between group dark:bg-slate-800 dark:hover:bg-slate-800"
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <div class="text-left">
                    <div class="font-medium text-gray-900 dark:text-slate-100">{{ collection.name }}</div>
                ...
                <Icon 
                  ...
                  class="text-xl text-gray-400 group-hover:text-gray-600 transition-colors dark:text-slate-500" 
                />
              </button>
            </div>
          </div>

          <div v-else-if="!showNewCollectionInput" class="text-sm text-gray-500 text-center py-8 dark:text-slate-400">
            No collections yet. Create one to get started.
          </div>
        </div>

        <div class="p-6 border-t border-gray-200 flex items-center justify-end dark:border-slate-700">
