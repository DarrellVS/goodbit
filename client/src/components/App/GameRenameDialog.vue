...
<template>
  <DialogRoot :open="open" @update:open="emit('update:open', $event)">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-md flex flex-col outline-none"
      >
          ...
          <div class="rounded-lg bg-gray-50 border border-gray-200 p-3 space-y-1">
            <div class="flex items-start gap-2 text-sm text-gray-700">
              ...
              <div>
                ...
                <span class="ml-1 font-mono text-gray-900">{{ folderName }}</span>
                <span class="text-gray-500 ml-1">(unchanged)</span>
              </div>
            </div>
            <div class="text-xs text-gray-600 ml-7">
        ...
        <div class="p-6 border-t border-gray-200 flex items-center justify-end gap-3">
          <DialogClose as-child>
            <button
              class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              :disabled="loading"
              @click="handleCancel"
            >
              Cancel
