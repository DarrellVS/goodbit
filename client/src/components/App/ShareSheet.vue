...
<template>
  <DialogRoot :open="open" @update:open="emit('update:open', $event)">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-sm flex flex-col outline-none modal-content-animate"
      >
        <div class="p-6 border-b border-gray-200">
          <DialogTitle class="text-xl font-bold text-gray-900 mb-1">Send to my phone</DialogTitle>
          <DialogDescription class="text-sm text-gray-600 truncate">{{ title }}</DialogDescription>
        </div>
        ...
        <div class="p-6 flex flex-col items-center gap-4">
          <div class="p-3 bg-white rounded-xl border border-gray-200">
            ...
            />
            <div v-else class="w-56 h-56 flex items-center justify-center text-sm text-gray-400">
          ...
          <div class="w-full flex items-center gap-2">
            <code class="flex-1 text-xs font-mono bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 truncate">
              {{ url }}
            </code>
            <button
              class="px-3 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors flex-shrink-0"
              title="Copy the link"
              @click="copy"
            >
