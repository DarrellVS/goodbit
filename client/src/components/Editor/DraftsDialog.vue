...
<template>
  <DialogRoot :open="open" @update:open="emit('update:open', $event)">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-lg flex flex-col outline-none modal-content-animate max-h-[85vh]"
      >
        <div class="p-6 border-b border-gray-200 flex items-start justify-between gap-4">
          <div>
            <DialogTitle class="text-xl font-bold text-gray-900 mb-1">Drafts</DialogTitle>
          ...
          <button
            class="px-3 py-2 rounded-lg border border-gray-300 text-gray-700 text-xs font-medium hover:bg-gray-50 transition-colors inline-flex items-center gap-1.5 flex-shrink-0"
        ...
        <div v-if="pendingImport" class="p-6 border-b border-gray-200 bg-orange-50/60 space-y-2">
          ...
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
            ...
            <button
              class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-white transition-colors"
              @click="cancelImport"
            >
              Cancel
            </button>
        ...
        <div class="p-6 border-b border-gray-200 space-y-2">
          ...
          <p v-if="!canSave" class="text-xs text-gray-500">
            Add a clip or a track before saving a draft.
          </p>
          <p v-else-if="activeId" class="text-xs text-gray-500">
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <div v-if="drafts.length === 0" class="text-center py-6">
            ...
            <p class="text-sm font-medium text-gray-700">No saved drafts</p>
          ...
          <ul v-else class="space-y-2">
            <li
              ...
              :key="draft.id"
              class="flex items-center gap-2 p-3 rounded-lg border border-gray-200 hover:border-orange-400 transition-colors"
            >
        ...
        <div class="p-6 border-t border-gray-200 flex justify-end">
          <DialogClose as-child>
            <button class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors">
              Done
            </button>
          </DialogClose>
        </div>
