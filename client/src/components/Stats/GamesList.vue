...
<template>
  <div class="bg-white border border-gray-300 rounded-xl p-6">
    ...
    <div v-if="games.length === 0" class="text-center py-12 text-gray-400">
      No games yet
    </div>
    ...
    <div v-else class="space-y-2 max-h-96 overflow-y-auto">
      <button
        v-for="game in games"
        :key="game.game"
        class="w-full text-left p-3 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer"
        @click="handleGameClick(game.game)"
      >
        <div class="flex items-center justify-between mb-1">
          <span class="font-medium text-gray-900">{{ game.game }}</span>
          <span class="text-sm text-gray-500">{{ game.count }} clips</span>
        </div>
