...
<template>
  <div class="p-6 space-y-6 max-w-7xl mx-auto mt-12">
    <div class="embla relative group">
      ...
      <button
        class="absolute -left-16 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-full w-11 h-11 flex items-center justify-center bg-white/60 dark:bg-neutral-800/70 backdrop-blur border border-white/40 dark:border-white/10 shadow-md hover:shadow-lg hover:bg-white/80 dark:hover:bg-neutral-700/80"
        :class="{ 'opacity-40 cursor-not-allowed': !canPrev }"
        :disabled="!canPrev"
        aria-label="Previous"
        @click="prev"
      >
        <Icon icon="radix-icons:chevron-left" class="text-neutral-800 dark:text-neutral-200 text-xl" />
      </button>
      <button
        class="absolute -right-16 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-full w-11 h-11 flex items-center justify-center bg-white/60 dark:bg-neutral-800/70 backdrop-blur border border-white/40 dark:border-white/10 shadow-md hover:shadow-lg hover:bg-white/80 dark:hover:bg-neutral-700/80"
        :class="{ 'opacity-40 cursor-not-allowed': !canNext }"
        :disabled="!canNext"
        aria-label="Next"
        @click="next"
      >
        <Icon icon="radix-icons:chevron-right" class="text-neutral-800 dark:text-neutral-200 text-xl" />
      </button>
    </div>
    ...
    <div class="mt-6 flex flex-wrap justify-center gap-4">
      <div v-for="(clip, idx) in todayClips" :key="'thumb-'+clip.id" class="group transition-all cursor-pointer w-40" @click="goTo(idx)" :aria-current="selectedIndex === idx ? 'true' : 'false'" :class="{ 'scale-105': selectedIndex === idx }">
        ...
        <div class="text-sm mt-2 text-muted-700 dark:text-muted-300 text-center">
          {{ clip.game }}
        </div>
      </div>
    </div>
