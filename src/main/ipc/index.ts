...
export function registerIpc(getWindow: () => BrowserWindow | null): void {
  ...
  /** Launch a game through Steam. The rules are in `services/steam/launch.ts`. */
  ipcMain.handle('steam:launch', async (_event, game: string) => {
    const { launchSteamGame } = await import('../services/steam/launch.js');
    return launchSteamGame(String(game));
  });
  ...
  /** Every notch tile with what it says right now, so the layout editor draws real ones. */
  ipcMain.handle('notch:tiles', async () => {
    const { notchTilesPreview } = await import('../services/notch/index.js');
    return notchTilesPreview();
  });
