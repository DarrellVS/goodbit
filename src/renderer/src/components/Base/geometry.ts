...
/**
 * A menu's frame, its separator, and the glyph at the start of each row.
 *
 * Variant-less, so they stay strings here rather than becoming recipes in
 * `variants.ts`: the rule is that anything with kinds is a `cva` recipe and
 * anything with one look is a constant. The rows themselves have kinds, and
 * are `menuItemVariants`.
 */
export const MENU_CONTENT =
  'min-w-[232px] bg-card rounded-md p-1.5 shadow-pop border border-border outline-hidden z-50';
export const MENU_SEPARATOR = 'h-px bg-border my-1.5 mx-2';
export const MENU_ICON = 'size-4 shrink-0 block text-muted-400';
...
/*
 * Buttons are not here any more. They have kinds, so they are a `cva` recipe,
 * `buttonVariants` in `variants.ts`, and the component is `BaseButton`.
 */
