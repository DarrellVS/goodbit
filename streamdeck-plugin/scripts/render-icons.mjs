...
function keySvg({ icon, colour, small }) {
  // On a 256 canvas: a 144 glyph centred, or for stats a 76 one near the top,
  // leaving the lower half for the two lines of count the key writes there.
  const box = small ? 76 : 144;
  const x = (256 - box) / 2;
  const y = small ? 22 : x;
  const scale = box / 256;
...
/*
 * The glyphs a key draws at run time, over a clip's own thumbnail: the discard
 * key shows what it would throw away, with a trash mark, or a lock when the
 * clip would be kept. Written as path data so the plugin needs no SVG files.
 */
const glyphs = Object.fromEntries(['trash', 'lock'].map((name) => [name, glyph(name)]));
writeFileSync(
  join(root, 'src', 'glyphs.ts'),
  `// Generated by scripts/render-icons.mjs from Phosphor (MIT). Do not edit.
